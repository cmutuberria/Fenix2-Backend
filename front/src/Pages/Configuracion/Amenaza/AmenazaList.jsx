...
export default ({ history }) => {
  ...
  const list = async () => {
    try {
      ...
      const result = await apiCall(`/amenaza?${params}`, null, null, "GET");
  ...
  const handleAdd = () => {
    history.push(`/Configuracion/Amenaza/Formulario`);
  };
  const handleEdit = (e, obj) => {
    history.push(`/Configuracion/Amenaza/Formulario/${obj._id}`);
  };
  ...
  const handleOkDelete = async () => {
    try {
      dispatch({ type: LOADING_START });
      setOpenDialog(false);
      const result = await apiCall(`/amenaza/${obj._id}`, null, null, "DELETE");
      dispatch({ type: LOADING_END });
      if (result) {
        list();
          ...
          <TableBody>
            {data.map((row, i) => (
              <TableRow key={i}>
                ...
                <TableCell>{row.tipo?"Amenaza":"Estrés"}</TableCell>
