...
export default ({ history }) => {
    ...
    const handleOkDelete = async () => {
        try {
            dispatch({ type: LOADING_START });
            setOpenDialog(false);
            const result = await apiCall(`/colector/${obj._id}`, null, null, 'DELETE');
            dispatch({ type: LOADING_END });
            if (result) {
                list();
    ...
    return (
        <React.Fragment>
            <div className={classes.header}>
                <Typography variant="h5" className={classes.title}>Listado de Colectores</Typography>
                <Button color="primary" variant="contained"
                    className={classes.btnMargin}
                    onClick={handleAdd}>Nuevo Colector</Button>
            </div>
            ...
            <Dialog title="Eliminar Colector" key="dialog"
                body="¿Desea eliminar el Colector?"
                open={openDialog}
