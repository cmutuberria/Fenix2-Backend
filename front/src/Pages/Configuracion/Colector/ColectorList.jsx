...
export default ({ history }) => {
  ...
  const list = async () => {
    try {
      ...
      const result = await apiCall(`/colector?${params}`, null, null, "GET");
  ...
  const handleAdd = () => {
    history.push(`/Configuracion/Colector/Formulario`);
  };
  const handleEdit = (e, obj) => {
    history.push(`/Configuracion/Colector/Formulario/${obj._id}`);
  };
    ...
    <React.Fragment>
      <div className={classes.header}>
        <Typography variant="h5" className={classes.title}>
          <FormattedMessage id="page.colectores.list.title" />
        </Typography>
        <Button
          color="primary"
          variant="contained"
          ...
          onClick={handleAdd}
        >
          <FormattedMessage id="page.colectores.list.btn_new" />
        </Button>
      </div>
      <TableContainer component={Paper}>
        <TableTextFilter
          ...
          placeholder={intl.formatMessage({
            id: "page.colectores.list.search",
          })}
        />
      ...
      <Dialog
        title={intl.formatMessage({ id: 'page.colectores.list.dialog.title' })}
        key="dialog"
        body={intl.formatMessage({ id: 'page.colectores.list.dialog.body' })}
        open={openDialog}
