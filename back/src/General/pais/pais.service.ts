...
import * as Paises from '../../Datos/Paises.json';
@Injectable()
export class PaisService {
  ...
  async createMany() {
    try {
      const elements = Paises.map(pais => {
        return new this.model({ nombre: pais.name, sigla: pais.sigla });
      });
      const all = await this.model.insertMany(elements);
      return all;
    } catch (error) {
      console.log(error);
    }
